...
대신 **3번이 실제로 위험한 진짜 문제**라서, 스펙 §6-1이 이미 "읽기 동작이라 자동화해도
...
| 스킵 랭커·100명 도달·매칭 실패 검출(스펙 §6-4 원문) | ➡️ **스킵 랭커·100명 도달은 이미 스킬 1(`arena-reward-table`)에 구현돼 있음** — 완료 시즌에 대해 `arena-reward-table`을 실행하면 이 둘이 나온다. 단, 매칭 실패 검출은 스테이킹(agentAddress)만 나오고 용기패스(avatarAddress) 매칭 실패는 스킬 1에도 아직 구현돼 있지 않다(`arena-reward-table` SKILL.md 참고). 이 스킬이 새로 만들 필요는 없었다 |
| 지급 후 tx 성공 여부 확인(스펙 §6-1) | ✅ **완료** — 실제 tx로 라이브 검증(`0f2623c4...`, SUCCESS, block 19,499,632) |
