...
## 칸 수가 어긋난 행 신호 (2026-09-03 "조용한 OK" 점검으로 추가)

셀이 밀린 CSV를 넣으면 diff는 **정직하게** 밀린 값을 보여준다 — 예를 들어 `Name` 칸이 통째로
빠진 행은 `Name "slime"→"100", HP "100"→""`로 나온다. 문제는 그게 **데이터 변경인지 파일이
깨진 건지 구분해주지 않는다**는 점이다. QA 담당자가 "이번에 이름을 100으로 바꿨나 보다"로
오해할 수 있다.

이 스킬에서 구조 검증을 다시 구현하지는 않는다(`datasheet-validate`의 몫이다). 대신 헤더와
칸 수가 다른 행을 세어 `[!] … 헤더와 칸 수가 다른 행 발견 — 이후(after) N행 (위 값 변경이
실제 변경이 아니라 셀이 밀린 결과일 수 있습니다. datasheet-validate로 구조를 먼저 확인하세요)`
한 줄로 알린다 — 판단은 그 도구로 넘긴다.

점검에서 함께 확인한, **문제 없던 것들**: 없는 파일(exit 1), 키 컬럼 부재·오타(exit 1),
빈 파일(exit 1), 헤더만 있는 파일(전량 추가/삭제로 정직하게 보고), 키 중복(`[!]`로 이미 알림).
