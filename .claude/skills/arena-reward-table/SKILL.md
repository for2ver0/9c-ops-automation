...
description: Nine Chronicles 아레나 시즌 상금 표를 계산하고 검증할 때 사용. RankingPool·배분 비율·보너스 배수 3종(스테이킹 lv2/lv3, 용기패스)을 받아 10개 랭크 구간별 지급액 표를 백오피스(ArenaRewardService)와 동일한 로직으로 재현하고, 백오피스가 검증하지 않는 불변식(인원 합 500, 비율 합 100%, 실지급 ≤ 총 풀, 그리고 2026-09-03 추가된 입력 정의역 검사: RankingPool > 0 · 보너스 배수 3종 > 0)과 위험 항목(테이블 밖 랭크 스킵, 용기패스 프리미엄 100명 이상 도달, 스테이킹 매칭 실패)을 검출한다. "아레나 상금 표 만들어줘", "이번 시즌 상금 계산해줘", "상금 표 검증해줘" 같은 요청에 사용. 동점으로 인한 구간 경계 이동은 골든 픽스처에 미착수(WARN) 항목으로만 기록돼 있고, 용기패스(avatarAddress) 매칭 실패 탐지는 아직 구현되지 않았다(§4 참고). 시즌 등록(ManageSeasons)이나 정산 실행(NCG 지급 서명·전송)은 이 스킬의 범위 밖이다.
---
...
- **무엇을 하는 스킬인가**: RankingPool·그룹별 배분 비율·보너스 배수 3종을 사람이 입력하면, 그걸로 10개
  ...
  스킵, 프리미엄 100명 상한, 스테이킹 매칭 실패, C# decimal ↔ JS double 반올림 경계값 — §4-1)을
  표면화하는 게 이 스킬의 존재 이유다. 동점 경계 이동과 용기패스 매칭 실패는 아직 탐지 로직이 없다
  (아래 4번 항목, §4 참고).
...
- **꼭 알아야 할 핵심 사실 6가지** (근거는 각 단계·`references/`에):
    ...
    4. 매칭 키가 다르다 — **스테이킹은 agentAddress, 용기패스는 avatarAddress** 기준. 매칭 실패
       리포트는 현재 스테이킹(agentAddress) 쪽만 낸다 — 용기패스(avatarAddress) 매칭 실패 탐지는
       아직 구현되지 않았다.
...
| `--table-only`에 유저별 CSV 플래그를 줘도 아무 신호 없이 무시됨 | 관찰만 함(2026-09-03 "조용한 OK" 점검) — `--table-only --staking-csv ./없는파일.csv`가 에러 없이 정상 표를 낸다. 그 모드는 원래 유저별 데이터를 안 쓰므로(§1) **문서상 설계와는 일치**해, 결함으로 단정하지 않고 기록만 남긴다 | 무시되는 플래그를 WARN으로 알릴지 담당자 판단 필요 |

## 입력 정의역 검사 (2026-09-03 "조용한 OK" 점검으로 추가)

기존 불변식은 전부 **"합이 맞는가"**만 봤다. 그래서 합 자체는 성립하는 병적인 입력이 전부
OK로 통과했다 — 실제로 돌려서 확인한 것:

| 입력 | 그때 결과 | 실제로 나온 표 |
| --- | --- | --- |
| `--pool 0` | 불변식 4개 전부 OK, exit 0 | 전원 0 NCG |
| `--pool -400000` | 불변식 4개 전부 OK, exit 0 | 1-1 그룹 **-28,000 NCG** 등 전원 마이너스 |
| `--staking-lv2 -2` | 불변식 전부 OK, exit 0 | 그 등급만 **-9,334 NCG** |

합 검사가 구조적으로 못 잡는 종류다 — 0의 합은 0이고 음수의 합은 그 음수라, "sum(groupReward)
== RankingPool"이 참이 되기 때문이다. 이 표는 그대로 PNG가 되어 디스코드 공지로 나가므로
계산 단계에서 막는다.

- `pool-positive` — `RankingPool > 0`이 아니면 FATAL.
- `multipliers-positive` — 보너스 배수 3종이 하나라도 `> 0`이 아니면 FATAL.

⚠️ **배수 하한을 1이 아니라 "0 초과"로 잡은 이유**: 골든 픽스처의 실측 관측값이
`stakingLv2Multiplier=0.5`(Odin S39·Heimdall CS9 공통), `couragePassMultiplier=1.2`(Heimdall
CS9)로 **1 미만 배수가 실재한다.** `>= 1`로 잡았으면 정상 시즌 설정을 오탐했을 것이다.

두 검사는 골든 픽스처의 기대 불변식 목록에도 추가해, 알려진 정상 config에서 계속 OK로
남는지 회귀로 지킨다(Odin S39 6개 / Heimdall CS9 7개).
