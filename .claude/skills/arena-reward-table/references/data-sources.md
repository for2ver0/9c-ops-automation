...
- ⚠️ **`/leaderboard/completed`는 `cached-block-info`의 캐시된 블록 높이를 기준으로 "완료 여부"를 판정하는 것으로 보임.** 실측(2026-08-30): 캐시된 `currentBlockIndex=19,498,315`가 실제 최신 시즌(45번, 시작 블록 20,081,624)보다 58만 블록 이상 뒤처져 있었고, 이 상태에서 시즌 41·42(`endBlock` 기준으로는 이미 끝났어야 함)를 조회하면 `400 "The requested block index corresponds to an ongoing or future season."`을 반환했다. 반면 더 오래된 완료 시즌(36·38·39·40)은 정상 응답. **→ 시즌 종료 직후 이 API를 호출하면 캐시 지연으로 일시적 400이 날 수 있다.** 이 400은 "영구 실패"가 아니라 "아직 캐시가 안 따라옴"일 가능성이 높으므로, 재시도(지수 백오프) 후에도 실패하면 CSV 폴백으로 넘어가야 한다.
