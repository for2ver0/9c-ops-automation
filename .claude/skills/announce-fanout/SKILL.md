...
| 출시일이 과거가 아님(KST) | 오늘 이후 | 오늘보다 과거(오타·재사용 의심), 또는 날짜가 실재하지 않아 판정 불가 (2026-09-05) | — |
| 출시 시각이 과거 관측치와 다름 | (해당 없음 — 아래 참고) | `--release-time`을 관측치 `11:00 AM`과 다른 값으로 명시(틀렸다는 뜻 아님, 확인만 요청) | — |

⚠️ 과거 여부 검사도 같은 함정이 있었다(2026-09-05 수정). 날짜가 실재하지 않으면 과거인지
계산할 수 없는데, 예전엔 그게 OK로 떨어지면서 `Date.UTC`로 굴러간 날짜를 "정상 범위"라고
단언했다 — `--release-date 2026-02-30`이 "March 2, 2026 — 오늘 이후로 정상 범위"라고 나왔다
(사람이 친 적 없는 날짜인 데다 실제로는 과거였다). 지금은 WARN "판정하지 않았습니다"로 나오고
굴러간 날짜를 보여주지 않는다. 날짜 자체는 위 FATAL이 원본 그대로 보여준다.
