...
description: Nine Chronicles 밸런스 구글 시트를 CSV로 내보낸 결과의 구조적 결함(중복 헤더, 빈 헤더, 행별 컬럼 수 불일치, 데이터 0행, 키 컬럼 공백, 키 값 중복, 행 수 급감, lib9c 스킵 행, 따옴표 형식)을 검증하고, `--baseline-csv`로 직전 회차 CSV를 넘기면 회차 간 diff(추가·삭제·변경 행/컬럼)까지 보여줄 때 사용. `--url`로 구글 시트를 직접 읽을 땐 gviz `headers=1`(없으면 대량 행 유실)을 자동으로 붙여 요청하고, 탭 이름 오타로 인한 기본 탭 폴백은 경고한다. "이 시트 검증해줘", "이 CSV 업로드해도 되는지 확인해줘", "지난 회차랑 뭐가 다른지 보여줘", "datasheet-validate 돌려줘" 같은 요청에 사용. 밸런스 시트는 무인증 공개 상태로 확인돼 승인 없이 바로 실행 가능하다(docs/9c-update-automation-permission-request.md ① 참고). ⚠️ 원래 설계 문서가 그리던 4가지 검증(시트 사전검증·시트 간 참조 ID·회차 diff·CSV 파싱부) 중 이 스킬은 시트 간 참조 ID 검증만 범위 밖이다 — lib9c 스키마 매핑이 선행돼야 해서다(아래 참고). 나머지 3가지(사전검증·회차 diff·CSV 파싱부)는 구현됐다.
---
...
8. **키 값 중복** (2026-09-03) — 같은 키 값이 두 번 이상 나오면 **WARN**. 행 수는 그대로라
   ...
   알리고 사람이 판단하게 한다.
9. **gviz headers 파라미터 / 요청한 탭 확인** (2026-09-03, `--url` 모드 전용) — `headers=`가
   없으면 **요청 직전에 도구가 `headers=1`을 자동으로 붙이고** 그 사실을 알린다(붙이지 못한
   경우에만 WARN). `sheet=`가 붙은 요청의 응답이 `sheet=`를 뺀 기본 탭 응답과 같으면
   WARN(탭 이름 오타 의심). 둘 다 §2의 ⚠️ 항목 참고.
10. **lib9c 스킵 행** (2026-09-03) — lib9c `Sheet.Set`이 건너뛰는 행(`_`로 시작, 그리고 gviz
   프록시에서만 첫 칸이 빈 행)을 세어서 **몇 행을 판정에서 뺐는지 항상 보고한다.** `_` 주석만
   있으면 정보성 **OK**(lib9c TableCSV 140개 중 7개가 쓰는 정상 관행이라 상시 WARN을 띄우면
   진짜 경고를 무시하게 된다), 첫 칸이 빈 행이 섞였을 때만 **WARN**(주석인지 id를 잃은
   데이터 행인지 모양으로 구분이 안 돼 사람이 판단해야 한다).
11. **따옴표 형식** (2026-09-03) — 모든 줄이 `"`로 시작하면 **WARN**. gviz 출력의 형태이고,
   lib9c 파서는 따옴표를 해석하지 못해 그대로 업로드하면 로드가 중단된다(§"--url 모드는
   프록시다" 참고). 원문을 못 받아 **확인하지 못한** 경우도 WARN이다 — 미실행을 OK로
   위장하지 않는다.
...
⚠️ **`&headers=1`은 스킬이 자동으로 붙인다** (2026-09-03 추가) — URL을 만들 때 신경 쓰지
않아도 된다. 이 파라미터가 없으면 구글이 헤더 행 수를
...
결과는 탭에 따라 무의미할 수 있다.** 지금은 `headers=`가 없는 URL을 받으면 요청 직전에
`headers=1`을 붙여 가져오고(`withGvizHeaders`), 그렇게 보충했다는 사실을 검사 결과에 적는다.
`headers=2`처럼 **유효한 숫자**가 이미 있으면 사용자의 의도로 보고 덮어쓰지 않는다.
다만 값이 비었거나(`&headers=`) 숫자가 아니면 실수로 보고 1로 덮어쓴다 — 구글이 그런 값을
무시하고 헤더 행 수를 추측해버려서, 존중하면 막으려던 유실이 그대로 재발하기 때문이다
(실측: `&headers=`로 요청하면 895행짜리 `CollectionSheet`가 다시 13행이 된다).
...
| 빈 헤더 | 없음 | 이름 없는 열이 있으나 그 열 데이터도 전부 비어 있음(구글 export 아티팩트) | 이름 없는 열에 **값이 들어있음**(줄 번호까지 표시) — 업로드 시 한 키로 뭉개져 소실. 데이터 내용을 판정하는 검사 중 **스킵 행을 판정 대상에서 빼지 않는 것은 이것뿐**이다 — 무명 열의 값은 어느 행에 있든 표 밖에 입력된 이물질이라서다(실제 lib9c CSV 140개 전부 무명 헤더 0개, 파서로 확인) |
...
| gviz headers 파라미터 | `headers=` 지정됨 / 도구가 자동으로 붙임 / URL 아님 | 자동 보충이 실패한 경우 | — |
...
| lib9c 스킵 행 (2026-09-03) | 건너뛸 행 없음, 또는 `_` 주석 행만 있음(정보성 — lib9c TableCSV 140개 중 7개가 쓰는 정상 관행) | 첫 칸이 빈 행이 섞임(주석일 수도, id를 잃은 데이터 행일 수도 있어 사람이 판단) | — |
| 따옴표 형식 (2026-09-03) | 따옴표로 감싸인 형식이 아님(업로드 가능한 형태) | **모든** 줄이 `"`로 시작(gviz 출력 형태 — 그대로 업로드하면 lib9c 로드가 중단됨), 또는 **원문을 못 받아 확인 불가**("미실행 ≠ 정상") | — |
...
| 2026-09-03 이전에 URL 모드로 돌린 검증 결과 | **신뢰할 수 없음 — 영향 탭 특정 완료** | 데이터 탭 10개 중 7개 영향, 그중 5개는 92~99% 행 유실. 해당 탭은 다시 돌릴 것(파라미터는 자동 보충된다 — 상세: "어느 탭이 영향받았나" 절) |
...
행 수는 **원본 줄 수**(주석·스킵 행 포함)다 — 아래 재검증 표는 lib9c 스킵 행을 뺀 수라 같은
탭이 다른 숫자로 보인다(예: `CostumeItemSheet` 172행 vs 166행).
...
- 위 7개 탭을 URL 모드로 검증한 기록이 있다면 **그냥 다시 돌릴 것** — 파라미터는 이제 스킬이
  자동으로 붙이므로 URL을 손볼 필요가 없다.
...
아래는 **주석 행 처리를 고친 뒤 다시 돌린 현재 출력**이다. 고치기 전에는
`EventScheduleSheet`/`[Heimdall]`/`CostumeItemSheet`이 주석 행 때문에 "키 공백 FATAL"·"키 값
중복"으로 잡혔는데, 그건 도구의 오탐이었다(주석 행은 lib9c가 로드하지 않는다).

10개 탭 **전부**가 `따옴표 형식` WARN을 낸다 — gviz 출력의 성질이라 탭별 특징이 아니므로 표에서 뺐다.
키 컬럼은 탭마다 다르다(`ItemRequirementSheet`=`item_id`, `EventDungeonStageWaveSheet`=`event_dungeon_stage_id`,
나머지 `id`) — 틀린 키를 주면 그 두 검사가 "헤더에 없음" WARN으로 조용히 건너뛴다.

| 탭 | 크기 | 따옴표 WARN 외 지적 |
| --- | --- | --- |
| `CollectionSheet` | 34칸 / 895행 | 없음 |
| `EventDungeonStageSheet` | 53칸 / 260행 | 없음 |
| `EventDungeonSheet` | 26칸 / 13행 | WARN — 이름 없는 빈 열 22개 |
| `MaterialItemSheet` | 26칸 / 330행 | WARN — 이름 없는 빈 열 21개 |
| `ItemRequirementSheet` | 26칸 / 568행 | WARN — 이름 없는 빈 열 23개 |
| `CostumeStatSheet` | 26칸 / 220행 | WARN — 이름 없는 빈 열 22개 |
| `CostumeItemSheet` | 26칸 / 166행 (스킵 6행 제외) | WARN — 이름 없는 빈 열 20개 |
| `EventDungeonStageWaveSheet` | 15칸 / 780행 | WARN — 키 값 중복 (**병합형 시트라 정상** — lib9c가 같은 id의 여러 행을 웨이브 목록으로 합친다) |
| `EventScheduleSheet` | 26칸 / 14행 (스킵 2행 제외) | WARN — 빈 열 16개 + lib9c 스킵 행 2개(2·3행, `_name` 열에 텍스트가 있는 주석 행) |
| `EventScheduleSheet[Heimdall]` | 26칸 / 11행 (스킵 3행 제외) | **FATAL — 이름 없는 14번째 칸 15행에 `" q"`** + 스킵 행 3개(2·3행은 주석, 15행은 이 오타뿐인 빈 행) |
...
전수 확인: 첫 칸이 빈 행(= lib9c 스킵 행)과 "이름 없는 열에 값이 있는" 셀을 10개 탭 전부에서
세어봤더니, `EventScheduleSheet`(스킵 2) · `[Heimdall]`(스킵 3 + 이물질 1) 외 **나머지 8개 탭은
모두 0**이었다.

### ⚠️ 처음 잡힌 FATAL 대부분은 데이터 결함이 아니라 **익스포트 경로 차이**였다

처음엔 "이 시트들이 지금 상태로 업로드되면 로드가 실패한다, 담당자 확인 필요"로 적었는데,
실제 업로드 아티팩트와 대조해보니 **틀린 결론이었다.** 아래 분석의 결과로 도구가 주석 행을
건너뛰도록 고쳐졌고, 그래서 위 표에는 그 오탐이 더 이상 나오지 않는다. 같은 행을 두 경로로
비교했다:
...
| 컬럼 추론 타입 | 주석 행의 첫 칸 | 고치기 전 판정(오탐) | 현재 판정 |
| --- | --- | --- | --- |
| number (`EventScheduleSheet.id`) | `null` → 빈 칸 | 키 컬럼 공백 **FATAL** | `lib9c 스킵 행` **WARN** — 빈 칸은 주석인지 id를 잃은 데이터 행인지 모양으로 구분이 안 돼 사람이 판단 |
| string (`CostumeItemSheet.id`) | 텍스트가 그대로 남음 | 키 값 중복 **WARN**(`_spine_resource_path` 4회) | `lib9c 스킵 행` **OK** — `_` 주석 6행으로 정확히 분류(정보성) |
...
그래서 2026-09-03에 도구가 이 행들을 판정에서 빼도록 고쳤고, 위 두 오탐은 더 이상 나오지
않는다(실측 재확인). 남은 `lib9c 스킵 행` WARN은 "빼고 판정했다"는 통지이지 결함 신호가
아니다 — 다만 그 행이 정말 주석인지는 여전히 사람이 확인해야 한다.
...
## `--url`(gviz) 모드는 프록시다 — 실제 업로드 파일과 형식이 다르다 (2026-09-03 운영 재확인)

`&headers=1` 누락 사고를 재확인하다 **더 근본적인 차이**가 드러났다. gviz 출력은 실제 업로드
아티팩트가 아니다:

| | gviz(`tqx=out:csv`) | 실제 lib9c TableCSV |
| --- | --- | --- |
| 따옴표 | 모든 필드를 감쌈 | **한 줄도 없음**(실측) |
| 주석 행 | 첫 칸이 빈 행으로 나타남 | `_`로 시작(`_item_sub_type,…`) |

lib9c의 `TryGetRow`는 `line.Trim().Split(',')`이 전부라 **따옴표를 이해하지 못하고**,
`row.Set(fields)`에 try/catch가 없어 파싱이 실패하면 **시트 로드 전체가 중단된다**(원본 확인).
즉 gviz CSV를 그대로 올리면 안 된다 — 이제 `따옴표 형식` 검사가 WARN으로 알린다.

### lib9c 스킵 규칙을 반영한다

`Sheet.Set`은 `if (line.StartsWith(",") || line.StartsWith("_")) continue;`로 주석 행을
건너뛴다. 이 스킬도 같은 규칙을 적용해 그 행들을 데이터 행 검사에서 제외하고, 몇 행을
그렇게 처리했는지 `lib9c 스킵 행` 항목으로 **항상 보고한다**(조용히 빼면 진짜 누락을 숨기게
된다). 판정은 `_` 주석만 있으면 정보성 **OK**, 첫 칸이 빈 행이 섞였을 때만 **WARN**이다.

⚠️ **빈 첫 칸은 형식에 따라 다르게 다룬다.** 실제 익스포트에는 `,`로 시작하는 줄이 0개라
(주석은 `_`만 씀) 거기서 빈 첫 칸은 **id를 잃은 데이터 행**이므로 `키 컬럼 공백` FATAL을
그대로 유지한다(v200450 실패 모드). gviz 프록시에서만 주석 행이 그렇게 보이므로 거기서만
제외한다.

### 재확인 결과 (2026-09-03)

`&headers=1` 없이 검증하던 시절 **10개 데이터 탭 중 7개가 영향**을 받았다. 위 "어느 탭이
영향받았나" 표와 같은 실측이고, 여기서는 유실률 순으로 다시 정렬한 것이다:

| 탭 | headers=1 없음 | 있음 | 유실 |
| --- | --- | --- | --- |
| CollectionSheet | 13행 | 895행 | 882 (99%) |
| ItemRequirementSheet | 6행 | 568행 | 562 (99%) |
| CostumeItemSheet | 3행 | 172행 | 169 (98%) |
| CostumeStatSheet | 6행 | 220행 | 214 (97%) |
| EventDungeonStageSheet | 20행 | 260행 | 240 (92%) |
| EventScheduleSheet[Heimdall] | 12행 | 14행 | 2 (14%) |
| EventScheduleSheet | 14행 | 16행 | 2 (13%) |

영향 없던 탭: `EventDungeonStageWaveSheet`, `EventDungeonSheet`, `MaterialItemSheet`.

올바른 URL로 다시 돌린 결과, 처음에 FATAL로 보였던 **키 공백 2건은 주석 행 오탐**이었다.
다만 **결함이 0건이었던 건 아니다** — `EventScheduleSheet[Heimdall]` 15행, 이름 없는 14번째
칸의 `" q"`는 실재하는 이물질이고 이 문서 "운영 조치"의 유일한 시트 수정 항목이다.
