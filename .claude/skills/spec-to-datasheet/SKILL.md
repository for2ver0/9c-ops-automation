...
실행: `bun test tools/9c/lib/spec-to-datasheet.test.ts` (16 pass).
...
| 같은 키를 가진 행이 2개 이상 | WARN | 어느 행이 정본인지 모호 — 전부 반영해야 함(아래 참고) |
...
1. **중복 키** — 시트에 같은 `Id`가 두 번 나오면 예전엔 **첫 행만** 보고 판단했다. 첫 행이
   이미 기획서 값이면 "작업 불필요"로 넘어가버리고, 옛 값을 가진 두 번째 행이 그대로 남는다.
   지금은 일치하는 행을 **전부** 보고, 하나라도 다르면 CHANGE로 잡으면서 발견된 값을 모두
   보여준다. 값이 전부 같아도 행이 여러 개면 WARN을 붙인다. 이 저장소는 원래 중복 키를 위험
   요소로 다뤄왔다(`qa-checklist`의 `duplicateKeys*` 추적, `datasheet-validate`의 "중복 키가
   있어 diff는 마지막 값만 반영함" 주의) — 그 관행에 맞춘 것이다.
