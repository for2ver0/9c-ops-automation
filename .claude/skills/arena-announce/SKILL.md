...
| ~~시작 시각 근접도 임계값(현재 3일)~~ | ✅ **해소됨** (2026-08-30) — 라이브 전체 22개 odin 시즌에 대해 "시작 시각이 가장 가까운 heimdall 시즌"을 자동 매칭해봤더니, 20/22건이 0~2.12일 이내였고 3일을 넘긴 2건 모두 이미 알려진 이상치였다: id=1 페어(777일 차이 — 정책 지문 조사에서 이미 더미/시드 row로 의심됐던 시즌들), odin g18↔heimdall g0(33일 차이 — heimdall 쪽이 바로 그 `seasonGroupId=0` 버그 시즌). 정상 페어는 전부 3일에 충분히 못 미쳐 임계값이 잘 잡혀 있음이 확인됨 | — |
