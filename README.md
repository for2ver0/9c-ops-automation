...
| [`datasheet-to-csv`](.claude/skills/datasheet-to-csv/SKILL.md) | 구글 시트 → lib9c `Lib9c/TableCSV/<시트명>.csv` 변환 + 현재 lib9c 값과 diff + 커밋 메시지 초안 | 2026-09-04 신규, 부분 구현 — lib9c 공개 커밋 이력을 직접 조사해 ⑦⑨를 해소하고 착수. gviz 대신 Sheets API로 읽고(손상 재수출 사고 2건과 같은 실패 모드 방지), lib9c 파일별 줄바꿈 스타일(LF/CRLF·트레일링 개행)을 그대로 맞춰 실제 5개 파일로 바이트 단위 왕복 검증 완료. 실제 fork push·PR 생성·git 실행은 항상 사람(범위 밖) |
| [`release-notes`](.claude/skills/release-notes/SKILL.md) | 버전+카테고리별 항목 → 깃북 붙여넣기용 릴리즈 노트 초안 | 부분 구현 — 버전 대사(+10 관행, 중복 게시 방지)·섹션 정리만. ⑥ 확인(깃북 에디터 직접 입력, GitHub 토큰 불필요)으로 착수. 문구는 짓지 않고 사람이 준 것만 정리. 2026-09-04: 헤딩 레벨(버전 `##`·카테고리 `###`)과 카테고리가 고정 목록이 아니라는 것을 실제 깃북 HTML 직접 대조로 확인 — 남은 한계는 콜아웃·이미지 같은 GitBook 전용 확장 문법 미검증(원본 저장소 `nine-chronicles-docs` 비공개)과, 실재가 확인됐지만 아직 표현 못 하는 3단 구조(`<h4>` 하위 카테고리)·불릿 아닌 문단형 회차 |
