...
| [`datasheet-validate`](.claude/skills/datasheet-validate/SKILL.md) | 밸런스 시트 CSV 구조적 검증 + 회차 간 diff(1차 필수) | 부분 구현 — 검사 9종: 중복 헤더·빈 헤더·행별 컬럼 수·데이터 0행·키 컬럼 공백·키 값 중복·행 수 급감(v200450 실패 모드 3종 회귀 포함)·`--baseline-csv` 회차 diff·gviz URL 점검(`headers=1` 누락, 탭 이름 오타 폴백). **뒤 4종은 2026-09-03 "조용한 OK" 점검으로 추가** — 특히 gviz `headers=1`이 없으면 실제 시트가 895행 → 13행으로 접히는데 그전엔 조용히 통과했다(그 이전 URL 모드 검증 결과는 신뢰할 수 없음). 시트 간 참조 ID·타입 검증만 lib9c 스키마 매핑 선행 필요로 미착수 |
